...
int nEnigmaNodeMinimum = 3;
std::set<CScript> sEnigmaAddresses;
extern void ProcessEnigma();

int nEnigmaReservedBalancePercent = 50;
...
extern map<uint256, CCloakingRequest> mapCloakingRequests;
extern CCriticalSection cs_mapCloakingRequests;

extern CCriticalSection cs_CloakingAccepts;
extern set<CCloakingAcceptResponse> sCloakingAccepts;

extern set<CCloakingSignResponse> sSignResponses;
extern CCriticalSection cs_sSignResponses;

extern set<CCloakingAcceptResponse> sCloakingAccepts;

extern set<uint256> sSignRequestHashes;
...
bool CTransaction::IsStandard() const
{
    ...
    {
        // Biggest 'standard' txin is a 6-signature 6-of-6 CHECKMULTISIG
        // pay-to-script-hash, which is 6 ~80-byte signatures, 6
        // ~65-byte public keys, plus a few script ops.
        if (txin.scriptSig.size() > 1024)
            return false;
                        ...
                        bool* pfMissingInputs)
{
    ...
    //if (tx.IsCoinStake())
    // TODO: remove this for after DDoS (single transaction sent as coin-stake) issue is fixed. This happens during
    // relaying of Enigma transactions and results in nodes being banned en-masse and network connectivity dropping.
    // See: "CTxMemPool::accept fails due to tx.IsCoinStake()"
    if (tx.IsCoinStake())
    ...
    if (fCheckInputs)
    {
        ...
        printf("CTxMemPool::accept(): in: %" PRI64d " Out: %" PRI64d "\n", tx.GetValueIn(mapInputs), tx.GetValueOut());
        int64 valIn = tx.GetValueIn(mapInputs);
        ...
        //printf("XXX CHECK TX size %ld", nSize);

        if (nFees < txMinFee) // return error("
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            if (!(fBlock && (nBestHeight < Checkpoints::GetTotalBlocksEstimate())))
            {
                ...
                if (!VerifySignature(txPrev, *this, i, fStrictPayToScriptHash, 0))
                {
                    ...
                    // enigma 3 accept trans fails here for input 0
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        nStakeMaxAge = 6 * 60; // test net min age is 6 min
    ...
    if (mapBlockIndex.empty())
    {
        ...
        block.nVersion = 1;
        block.nTime    = fTestNet ? blockTimeTestNet : blockTime;
        block.nBits    = bnProofOfWorkLimit.GetCompact(); // 1e0fffff
        block.nNonce   = fTestNet ? nNonceTestNet : nNonce;
        ...
        printf("hashMerkelRootOfficial == %s\n", hashMerkelRootOfficial.ToString().c_str());
...
bool ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion == 10300)
            pfrom->nVersion = 300;
        if (!vRecv.empty())
            vRecv >> addrFrom >> nNonce; //vRecv >> addrFrom >> verification_token >> nNonce;
        if (!vRecv.empty())
            vRecv >> pfrom->strSubVer;
        if (!vRecv.empty())
            vRecv >> pfrom->nStartingHeight;

        //printf("version: decoded verification_token: %"PRI64u"\n", verification_token);
	   ...
	   /* printf("version: addrman check verification token\n");
	    if (addrman.CheckVerificationToken(addrFrom, verification_token)) {
                ...
                addrman.Good(pfrom->addr);
	        printf("version: addrman verification token verified.\n");
            } else {
                printf("version: addrman could not verify verification token, setting reconnect token.\n");
        ...
        printf("receive version message: version %d, blocks=%d, us=%s, them=%s, peer=%s, verification=%" PRI64u "\n", pfrom->nVersion, pfrom->nStartingHeight, addrMe.ToString().c_str(), addrFrom.ToString().c_str(), pfrom->addr.ToString().c_str(), verification_token);

        printf("version: update peer block counts with peer starting height\n");
        cPeerBlockCounts.input(pfrom->nStartingHeight);

        printf("version: ask for pending sync checkpoint if any\n");
        // ppcoin: ask for pending sync-checkpoint if any
        if (!IsInitialBlockDownload())
            Checkpoints::AskForPendingSyncCheckpoint(pfrom);
    }
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // the reference block is now the best block, whatever the proof -- see Peershares, pull req #110
    ...
    else if (strCommand == "alert")
    {
        ...
        if (pfrom->setKnownAlerts.count(alertHash) == 0)
        {
    ...
    else if (strCommand == ENIMGA_STR_ANNOUNCE && fEnigma)
	{
        if(GetBoolArg("-printenigma"))
            ...
            uint256 annHash = ann.GetSigningHash();
            if (!pfrom || (pfrom->setKnownAnns.count(annHash) == 0  && !CCloakShield::GetShield()->IsEnigmaPeerBanned(ann.pEnigmaSessionKeyHex)))
            {
                if (ann.ProcessAnnouncement(pfrom))
                {
                    if(GetBoolArg("-printenigma"))
                        printf("ENIGMA: Processed received announcement, relaying to peers.\n");
                    // Relay
                    if(pfrom)
                        pfrom->setKnownAnns.insert(annHash);

                    mapEnigmaAnnNodes.insert(make_pair(annHash, pfrom));
                    {
                        LOCK(cs_vNodes);                        
                        BOOST_FOREACH(CNode* pnode, vNodes)
                            ann.RelayTo(pnode);
    ...
    if (pfrom != NULL && pfrom->fNetworkNode)
        if (pfrom != NULL && (strCommand == "version" || strCommand == "addr" || strCommand == "inv" || strCommand == "getdata" || strCommand == "ping"))
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (fProofOfStake)  // attempt to find a coinstake
    {
        ...
        if (nSearchTime > nLastCoinStakeSearchTime)
        {
            ...
            {
				if (txCoinStake.nTime >= max(pindexPrev->GetMedianTimePast()+1, pindexPrev->GetBlockTime() - GetMaxClockDrift(pindexPrev->nHeight+1)))
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake && !fStaking)
            continue; // staking disabled, skip
        ...
        if (fProofOfStake)
        {
            ...
            if(fEnigma)
            {
                try
                {
                    UpdateEnigmaAvailability();
                }
                catch(std::exception& e)
                {
                    if(GetBoolArg("-printposa"))
                        printf("StakeMiner: ProcessEnigma ex: %s\n", e.what());
                }
            }
	    ...
	    hash = pblock->GetHash();
            ...
            if (pblock->GetBlockTime() >= (int64)pblock->vtx[0].nTime + GetMaxClockDrift(pindexPrev->nHeight+1))
...
void UpdateEnigmaAvailability()
{
    ...
    if(nEnigmaCount >= nEnigmaNodeMinimum || true) // sort this out when we start onion routing anns
    {
        // If time since last update is longer than timeout seconds, broadcast announcement
        // we resend an updated announcement before the previous one expires to account for network delays
        // note: this logic will need to change once we introduce session key recycling!
        if(GetAdjustedTime() >= nLastEnigmaTime + CLOAKSHIELD_ANNOUNCE_TIMEOUT_SECS - (CLOAKSHIELD_ANNOUNCE_TIMEOUT_SECS/4))
        {
            if(GetBoolArg("-printenigma"))
                printf("ENIGMA: UpdateEnigmaAvailability\n");

            nLastEnigmaTime = GetAdjustedTime();
            broadcastenigma();
    ...
    {
        if(GetBoolArg("-printenigma"))
            printf("ENIGMA: UpdateEnigmaAvailability(): Must have 3 Enigma connections to participate.\n");
    }
}

void TrackEnigmaAddresses()
{
    BOOST_FOREACH(const PAIRTYPE(CBitcoinAddress, string)& item, pwalletMain->mapAddressBook)
    {
        ...
        {
            CKeyID myPosaKeyID;
            if (!address.GetKeyID(myPosaKeyID))
            {
                printf("ENIGMA: TrackEnigmaAddresses(): Address does not refer to a key\n");
            }else{
                CPubKey vchMyPosaPubKey;
                pwalletMain->GetPubKey(myPosaKeyID, vchMyPosaPubKey);
                CScript scriptMyPosaKey;
                scriptMyPosaKey.SetDestination(vchMyPosaPubKey.GetID());
                sEnigmaAddresses.insert(scriptMyPosaKey);
            }
        }
    }
}
