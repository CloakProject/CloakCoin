...
const uint256 maxStealth("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFEBAAEDCE6AF48A03BBFD25E8CD0364140");
const uint256 minStealth(16000); // increase? min valid key is 1
...
bool GenerateEnigmaSecret(const CStealthAddress &stealthAddress, const uint256 &sourceHash, const uint256 &cloakerHash, const unsigned int &index, ec_secret &secret)
{
    ...
    if (index >= 60){
        printf("Error: GenerateRandomSecret key %d requested, must be < 60.\n", index);
        return false;
    }

    uint256 test = stealthAddress.GetEnigmaHash(cloakerHash, sourceHash, uint256(index));

    // -- check max, try max 32 times
    // recursively hash until we are within bounds...
    int i;
    for (i = 0; i < 32; ++i)
    {
        test = Hash(test.begin(), test.end());
        if (test > minStealth && test < maxStealth){
            memcpy(&secret.e[0], test.begin(), 32);
            break;
        };
    };

    if (i > 31){
        printf("Error: GenerateRandomSecret failed to generate a valid key for %d.\n", index);
        return false;
    };

    //printf("Generated Enigma secret for %s with hash %s and index %d got %s\n", stealthAddress.Encoded().c_str(), cloakerHash.GetHex().c_str(), index, test.GetHex().c_str());
...
int StealthSecret(ec_secret& secret, const ec_point& pubkey, const ec_point& pkSpend, ec_secret& sharedSOut, ec_point& pkOut)
{
    ...
    */
    ...
    memcpy(&vchSecret[0], &secret.e[0], ec_secret_size);

    uint256 hashSecretIn = uint256(vchSecret);
    uint256 hashScanPubkey = uint256(HexStr(pubkey));
    uint256 hashSpendPubKey = uint256(HexStr(pkSpend));

    memcpy(&vchSecret[0], &sharedSOut.e[0], ec_secret_size);
    uint256 hashSecretOut = uint256(vchSecret);
    uint256 hashPubkeyOut = uint256(HexStr(pkOut));


    printf("SS %s - %s - %s - %s - %s\n",
           hashSecretIn.GetHex().c_str(),
           hashScanPubkey.GetHex().c_str(),
           hashSpendPubKey.GetHex().c_str(),
           hashSecretOut.GetHex().c_str(),
           hashPubkeyOut.GetHex().c_str()
           );
*/
    return rv;
};
