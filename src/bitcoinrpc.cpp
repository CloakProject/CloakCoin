...
{
    return GetBoolArg("-testnet", false) ? 29665 : 29661;
    //return fTestNet ? 29663 : 29661;
}
...
{ //  name                      function                 safemd  unlocked
  //  ------------------------  -----------------------  ------  --------
    ...
    { "sendcloakedtoaddress",   &sendcloakedtoaddress,   false,  false },
...
//    { "hexenigma", 		&hexenigma,		 true,   false },
    { "dumpenigmap", 		&dumpenigmap,		 true,   false },
//    { "enstr", 			&enstr,			 true,   false },
//    { "destr",	 		&destr,			 true,   false },
...
static string HTTPReply(int nStatus, const string& strMsg, bool keepalive)
{
    ...
    //CORS - Prefight Response
    if(nStatus == HTTP_OPTIONS){
        printf("Opt - strMsg=%s", strMsg.c_str());
        printf("Opt - nStatus=%d", nStatus);
        return strprintf("HTTP/1.0 200 OK\r\n"
            "Date: %s\r\n"
            "Server: CloakCoin-json-rpc/%s\r\n"
            "Access-Control-Allow-Origin: *\r\n"
...
void ThreadRPCServer3(void* parg)
{
    ...
    while (true) {
        ...
        //First check ACCESS-CONTROL header~
        if (mapHeaders.count("access-control-request-method") == 1)
        {
            printf("Access Control OPTIONS detected");
            conn->stream() << HTTPReply(HTTP_OPTIONS, "", false) << std::flush;
            break;
        }
...
Array RPCConvertValues(const std::string &strMethod, const std::vector<std::string> &strParams)
{
    ...
    if (strMethod == "sendcloakedtoaddress"   && n > 1) ConvertTo<double>(params[1]);
    ...
    if (strMethod == "getbigtx"     && n > 0) ConvertTo<boost::int64_t>(params[0]);
