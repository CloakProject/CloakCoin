...
bool fStaking = true;
bool fEnigma = true;
bool fEnigmaAutoRetry = true;
bool fEnableOnionRouting = true;
bool fDebug = false;
bool fDebugNet = false;
bool fDebugEnigma = false;
...
std::vector<int64> SplitAmount(int64 amount, int64 min, int64 maxparts)
{
    ...
    for(int i=0; i<maxparts; i++){
        int64 amt = GetRandRange(min, (amount - assigned) / (maxparts - i));
        if (assigned + amt < amount && amt > min){
            assigned += amt;
    ...
    if (assigned < amount){
        ...
        if (results.size() == maxparts || remain <= min){
            results.at(0) += remain;
        }else{
...
bool GetRandBool()
{
    return (bool)(GetRandInt(MAX_SIZE) % 2);
}
...
string FormatTimeFromSecs(int64 secs)
{
    string str = strprintf("%02d:%02d", (int)(secs/60%60), (int)secs%60);
    return str;
}
...
void AddTimeData(const CNetAddr& ip, int64_t nTime)
{
    ...
    //  set max absolute offset observed
...
std::string FormatSubVersion(const std::string& name, int nClientVersion, const std::vector<std::string>& comments, bool noSlashes)
{
