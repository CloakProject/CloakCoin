...
Value getbigtx(const Array& params, bool fHelp)
{
    ...
    while (pblockindex->pprev)
    {
        block.ReadFromDisk(pblockindex, true);
        BOOST_FOREACH(CTransaction& vtx, block.vtx)
        {
            int64 nValue = 0;
            int idx=0;
            BOOST_FOREACH(CTxOut& out, vtx.vout)
            {
                nValue+=out.nValue;

                if (out.nValue >= amount * COIN)
                {
                    Object entry;
                    entry.push_back(Pair("amount", ValueFromAmount(out.nValue)));
                    entry.push_back(Pair("txid", vtx.GetHash().GetHex()));
                    entry.push_back(Pair("vout", idx));
                    entry.push_back(Pair("confirmations", depth));
    ...
    while (pblockindex->nHeight > nBestHeight)
        pblockindex = pblockindex->pprev;
