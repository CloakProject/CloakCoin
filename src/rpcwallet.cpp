...
extern map<uint256, CCloakingRequest> mapOurCloakingRequests; // key = CCloakingIdentifier hash
extern CCriticalSection cs_mapOurCloakingRequests;
extern map<uint256, CCloakingRequest> mapCloakingRequests; // key = CCloakingIdentifier hash
...
Value getinfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("testnet",       fTestNet ? testnetNumber : 0));
...
Value sendcloakedtoaddress(const Array& params, bool fHelp)
{
    ...
    if (nAmount < 0)
        throw JSONRPCError(-103, "Send Enigma - Amount is zero.");
    ...
    if(nAmount > nBalance || (nAmount + (int64)reward + (int64)reserveForFees) > nBalance)
    {
        throw JSONRPCError(-102, "Send Enigma - Amount exceeds balance - need to stake??");
    }
    ...
    // *** get our inputs/outputs for the shared transaction ***
    CCloakingInputsOutputs inOuts;

    // create the request so we can reference the stealth hash
    ...
    //printf("XX nval %d, feeAndReward %d, total %d", nValue , feeAndReward, nValue + feeAndReward );

    if (!pwalletMain->SelectCoins(nAmount + reward + reserveForFees, GetTime(), setCoins, nValueIn, NULL))
        throw JSONRPCError(-105, "Send Enigma - Failed to select coins.");

    inOuts.nSendAmount = nAmount;
    ...
    int64 nChange = nValueIn - nAmount - reward;

    OutputDebugStringF(" nChange %" PRI64d " nValueIn %" PRI64d " nValue %" PRI64d " reward %" PRI64d " fee %" PRI64d "\n", nChange, nValueIn , nAmount, reward, reserveForFees );

    // store enigma address against request so we can assign our change later
    if (!pwalletMain->GetEnigmaAddress(req->senderAddress)){
        throw JSONRPCError(RPC_ENIGMA_NO_OWN_ADDRESS, "Send Enigma - Failed to find own Enigma address.");
    }

    // handle the inputOutput object
    CCloakingRequest* reqSent = Enigma::SendEnigma(inOuts, req);
    if (reqSent)
    {
        Object result;
        ...
        Object result;
        result.push_back(Pair("result", "Enigma request failed."));
        result.push_back(Pair("amount to cloak", FormatMoney(nAmount)));
        result.push_back(Pair("cloakers", 0));
        result.push_back(Pair("retry-enabled", 0));
        result.push_back(Pair("timeout secs", 0));
        result.push_back(Pair("request id", "0"));
        return result;
    }
}
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 5)
        ...
        "sendtoaddress <CloakCoinaddress> <amount> [comment] [comment-to] [narration]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
        "coins can be anonymized via Enigma by appending :enigma to the destination address, which will cloak the address you sent them from."
            + HelpRequiringPassphrase());
    ...
    string strError = pwalletMain->SendMoneyToDestination(address.Get(), nAmount, sNarr, wtx, false);
    if (strError != "")
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        ...
        "sendfrom <fromaccount>[:enigma] <toCloakCoinaddress> <amount> [minconf=1] [comment] [comment-to] [narration]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
        "coins can be anonymized via Enigma by appending :enigma to the account name, which will cloak the address you sent them from."
            + HelpRequiringPassphrase());
    ...
    if (iSeperator != std::string::npos)
    {
        ...
        bAnonymize = boost::iequals(action, "enigma");
    }
    ...
    // Send
    string strError = pwalletMain->SendMoneyToDestination(address.Get(), nAmount, sNarr, wtx, bAnonymize);
    if (strError != "")
        throw JSONRPCError(RPC_WALLET_ERROR, strError);
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    if (iSeperator != std::string::npos)
    {
        string action = strAccount.substr(iSeperator+1);
        strAccount = strAccount.substr(0, iSeperator);
        //bAnonymize = boost::iequals(action, "posa");
        bAnonymize = false;
    }
    ...
    int nChangePos;
    bool fCreated = pwalletMain->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, nChangePos, strFailReason, bAnonymize);
    if (!fCreated)
    {
...
Value ListReceived(const Array& params, bool fByAccounts)
{
    ...
    for (map<uint256, CWalletTx>::iterator it = pwalletMain->mapWallet.begin(); it != pwalletMain->mapWallet.end(); ++it)
    {
        ...
        if (wtx.IsCoinBase() || wtx.IsCoinStake() || !wtx.IsFinal() || wtx.IsEnigma())
            continue;
            ...
            if (mi != mapBlockIndex.end() && (*mi).second)
            {
                ...
                if (pindex->IsInMainChain())
                {
                    ...
                    if (tx.FetchInputs(txdb, mapUnused, false, true, mapInputs, fInvalid))
                    {
                        ...
                        char totalIn[255];
                        char totalOut[255];
                        sprintf(totalIn, "total in [%d]", (int)tx.vin.size());
                        sprintf(totalOut, "total out [%d]", (int)tx.vout.size());
...
Value walletpassphrase(const Array& params, bool fHelp)
{
    ...
    if (strWalletPass.length() > 0)
    {
        ...
        throw runtime_error(
            "walletpassphrase <passphrase> <timeout> [mintonly]\n"
            "Stores the wallet decryption key in memory for <timeout> seconds. Unlock for staking only with [mintonly]");
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid Cloak stealth address."));
        return result;
    };
...
Value scanforstealthtxns(const Array& params, bool fHelp)
{
    ...
    printf("Found %u new owned enigma transactions.\n", pwalletMain->nFoundEnigma);
...
/*
Value initiatecloak(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
        "initiatecloak <amount>\n"
        "<amount> is a real and is rounded to the nearest 0.000001\n"
        "this will create and broadcast a cloaking request and put it in the queue\n"
        "to be monitored and wait for participants to execute the ring escrow.\n"
                );

    // Amount
    double dAmount = params[0].get_value<double>();
    int64 nAmount = roundint64(dAmount * COIN);

    if (nAmount < MIN_TXOUT_AMOUNT)
        throw runtime_error("Send amount too small");

    //if ((nAmount/COIN) % 10 > 0)
    if(fmod(dAmount, 10) > 0)
	throw runtime_error("Cloak amount must be evenly divisible by 10.");

    Object result;
    result.push_back(Pair("result", "Cloaking request initiated."));
    result.push_back(Pair("Amount to cloak", dAmount));

    CCloakingRequest r;
    r.InitiateCloak(nAmount, CCloakShield::GetShield()->GetKey());
    ...
    LOCK(pwalletMain->cs_mapCloakingRequests);
    {
        BOOST_FOREACH(CloakingMapType::value_type p, pwalletMain->mapOurCloakingRequests)
        {
            CCloakingRequest r = p.second;
                if(r.nBroadcastedTime > 0)
                    result.push_back(Pair("CloakingRequest-Us", r.ToString()));
        }
        ...
        BOOST_FOREACH(CloakingMapType::value_type p, pwalletMain->mapCloakingRequests)
        {
            CCloakingRequest r = p.second;
                if(r.nBroadcastedTime > 0)
                    result.push_back(Pair("CloakingRequest-Them", r.ToString()));
        }
    }
    return result;
}
