...
public:
    ...
    void updateWallet(const uint256 &hash, int status)
    {
        OutputDebugStringF("updateWallet %s %i\n", hash.ToString().c_str(), status);
        {
            ...
            // posa
            if(wallet->setEnigmaProcessed.find(hash) != wallet->setEnigmaProcessed.end())
            {
...
QString TransactionTableModel::formatTxType(const TransactionRecord *wtx) const
{
    ...
    case TransactionRecord::Generated:
        return tr("Mined");
    case TransactionRecord::Enigma:
        if (abs(wtx->debit) > wtx->credit)
            return tr("Sent with Enigma");
        else
            return tr("Received with Enigma");
...
QVariant TransactionTableModel::txAddressDecoration(const TransactionRecord *wtx) const
{
    ...
    case TransactionRecord::Enigma:
        if (abs(wtx->debit) > wtx->credit)
            return QIcon(":/icons/tx_posa_output");
        else
            return QIcon(":/icons/tx_posa_input");
