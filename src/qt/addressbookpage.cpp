    ...
    tab(tab)
{
    ...
    ui->newCloakAddressButton->setIcon(QIcon());
...
void AddressBookPage::on_useForEnigmaEnableAction_clicked()
{
    ...
    model->updateEntry(address, label, true, CT_ENIGMA_ENABLED);
}

void AddressBookPage::on_useForEnigmaDisableAction_clicked()
{
    QString address = GUIUtil::getEntryData(ui->tableView, AddressTableModel::Address);
    QString label = GUIUtil::getEntryData(ui->tableView, AddressTableModel::Label);
...
void AddressBookPage::on_newCloakAddressButton_clicked()
{
    if(!model)
        return;
    EditAddressDialog dlg(
            tab == SendingTab ?
            EditAddressDialog::NewCloakSendingAddress :
            EditAddressDialog::NewCloakReceivingAddress, this);
    dlg.setModel(model);
    if(dlg.exec())
    {
        newAddressToSelect = dlg.getAddress();
...
void AddressBookPage::contextualMenu(const QPoint &point)
{
    ...
    QModelIndexList indexes = table->selectionModel()->selectedRows(AddressTableModel::Address);
    {
        foreach (QModelIndex index, indexes){
            if(index.isValid()){
                selectedAddresses.push_back(index.data().toString().toStdString());
            }
        }
    }
    bool showEnigmaEnable = false;
