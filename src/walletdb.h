...
public:
    ...
    bool WriteBlocksUntilEnigma(int64 nBlocksEnigma);
    ...
    bool WriteCloakingRequest(const CCloakingRequest request)
    {
        nWalletDBUpdated++;
        return Write(std::make_pair(std::string("posa3req"), request.identifier.GetHash()), request);
    }

    bool EraseCloakingRequest(uint256 hash)
    {
        nWalletDBUpdated++;
        Erase(std::make_pair(std::string("posa3req"), hash));
        return Erase(std::make_pair(std::string("posa3req"), hash));
    }

    bool EraseCloakingInOuts(uint256 hash)
    {
        nWalletDBUpdated++;
        return Erase(std::make_pair(std::string("posa3inout"), hash));
    }
    ...
    bool WriteEnigmaFeesEarnt(int64 feesEarnt)
    {
        nWalletDBUpdated++;
        return Write(std::string("posafeesearnt"), feesEarnt);
    }

    bool WriteActiveEnigmaAddresses(const std::set<std::string>& addresses)
    {
        nWalletDBUpdated++;
        return Write(std::string("activeenigmaadds"), addresses);
    }
